...
class VNN2Pointnet2(nn.Module):
    ...
    def __init__(self, pointnet_radius=0.1, hidden_dim=512, in_features=3,
        ...
        if out_features%3 != 0:
            print('This might break. The module expects a feature to be a multiple of 3.')
        self.vnn_resnet = VNN2_ResnetPointnet(c_dim=int(out_features/3), dim=in_features, device=device)

    def forward(self, pc):
        xyz_feats = self.vnn_resnet(pc)
        return xyz_feats.reshape(pc.shape[0], -1)    

class VNN2_ResnetPointnet(nn.Module):
    ''' DGCNN-based VNN vision_encoder network with ResNet blocks.
    ...
    def forward(self, p):
        ...
        index = p[:, :, [3], :] # (B, 1, 1, N)
        # Get NN graph (B, C, 3, N, K)
        feat = get_graph_feature_cross(pos, k=self.k, device=self.device)
        ...
        net = self.block_0(net)
        pooled = self.pool(net, dim=-1, keepdim=True).expand(net.size()) # (B, C, 3, 1)
        net = torch.cat([net, pooled], dim=1)

        net = self.block_1(net)
        weight = index / torch.sum(index)
        pooled = torch.sum(net * weight, dim=-1, keepdim=True).expend(net.size())
        # pooled = self.pool(net * index, dim=-1, keepdim=True).expand(net.size())
        net = torch.cat([net, pooled], dim=1)
        ...
        net = self.block_4(net)
        weight = index / torch.sum(index)
        net = torch.sum(net * weight, dim=-1)
        # net = self.pool(net * index, dim=-1)
        c = self.fc_c(self.actvn_c(net))
