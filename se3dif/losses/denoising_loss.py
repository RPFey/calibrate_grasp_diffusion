...
class ProjectedSE3DenoisingLoss():
    ...
    def __call__(self, model, model_input, ground_truth, val=False, eps=1e-5):
        ...
        # Compute L1 loss
        # TODO This is a bug, I do not think we should normalize the target (/ std)
        # The fixed version is in ProjectedFixedSE3DenoisingLoss
        z_target = z / std[..., None]
        loss_fn = nn.L1Loss()
...
class ProjectedFixedSE3DenoisingLoss():
    def __init__(self, field='denoise', delta = 1., grad=False):
        self.field = field
        self.delta = delta
        self.grad = grad
...
class DirichletSE3DenoisingLoss():
    def __init__(self, field='denoise', delta = 1., grad=False):
        self.field = field
        self.delta = delta
        self.grad = grad
...
class ProjectedSE3DenoisingCOSLoss():
    def __init__(self, field='denoise', delta = 1., grad=False):
        self.field = field
        self.delta = delta
        self.grad = grad
