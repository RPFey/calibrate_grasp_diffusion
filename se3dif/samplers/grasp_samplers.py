...
class Grasp_AnnealedLD():
    ...
    def _step(self, H0, t, noise_off=True):
        ...
        if noise_off:
            ...
            c_lr = 0.003 # cmax(0.05 * np.cos( (np.pi / 2) * t / self.T_fit), 1e-3) if self.enable_time else 0.05
...
class Grasp_PCSampler():
    ...
    def _step(self, H0, t, noise_off=True):
        ...
        if self.enable_time:
            ...
            phase_T_1 = max( ((self.T - (t + 1)) / (self.T)) + eps, eps)
        ...
        if noise_off:
            phase = self.model.final_t # + 1 / (2 * self.T)
            c_lr = 0.003 # max(0.05 * np.cos( (np.pi / 2) * t / self.T_fit), 1e-3) if self.enable_time else 0.05
